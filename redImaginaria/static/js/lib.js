...
var submit_form = function(e) {
	...
	$.post( 'http://reverso.local:5000/_conversar', {
		...
		lstMsj[ct] = data.mensaje;
		ct++;
		setTimeout(9600);
		console.log("Mensaje enviado: "+ data.mensaje);
		$('#mensajeParaEnviar').val('');
	 });
	 return false;
};
	...
	$('#mensajeParaEnviar').focus();
});
	var horaDeInicioSession = new Date();
	var lista = [];
	$.updater({
		//url: $SCRIPT_ROOT + '/_mensajes',
		url: 'http://reverso.local:5000/_mensajes',
		method: 'get',
		dataType: 'json',	
		interval: 1500
	},
	...
	//	$('#listaDeMensajesRecibidos ul').html("");
...
function updateList(horaDelMensaje) {
$.each(lstMsj,function(index,data) {
	console.log(lstMsj);
	if(lstMsj.length >=1){
	var msgChat = horaDelMensaje.getUTCDate() + "/" + horaDelMensaje.getMonth() + "/" + horaDelMensaje.getUTCFullYear() + " :: " +  data;
                                $('#listaDeMensajesRecibidos ul').prepend(
                                $('<li>').append(
                                $('<p>').append(
                                $('<span>').attr('class', 'mensajeRX').append(msgChat)
...
function esHoy( d1, d2 ){
	return true;
	console.log(d1.getUTCHours()+" "+d2.getUTCHours());
...
$(document).ready(function(){
	...
	$('#enviarMensaje #enviar').height(
		$('#enviarMensaje #mensaje').height()
	)
